...
//
// ■ md（768px）未満：カードをやめて白い画面に直接置く（案1「余白と見出し」）
//   docs/decisions/mobile-ui-no-boxes.md / docs/mockups/mobile-v1/V1Login.dc.html
//   枠・影・色つき背景でグループを囲まない。入力欄の背景だけは例外。
...
export const authActionClass =
  "flex h-13 w-full items-center justify-center gap-2 rounded-[26px] bg-navy font-en text-base " +
...
export default function AuthShell({
  ...
  return (
    <main
      className={
        "flex min-h-[calc(100svh-var(--bottom-nav-h))] flex-col bg-white px-4 " +
        "pt-[calc(var(--header-h)+40px)] pb-6 " +
        "md:min-h-screen md:items-center md:justify-center md:bg-bg-gray md:px-5 md:pb-15"
      }
    >
      <div
        className={
          "flex w-full flex-1 flex-col gap-8 " +
          "md:max-w-[440px] md:flex-none md:gap-0 md:rounded-xl md:border md:border-line-light " +
          "md:bg-white md:p-10 md:shadow-lg"
        }
      >
        {/* ロゴは md 以上だけ。md 未満は固定ヘッダーに同じロゴが出ているので重ねない。 */}
        ...
        <div className="flex flex-col gap-1.5 md:contents">
          <h1 className="text-[28px] font-black text-navy md:mb-1.5 md:text-center md:text-[1.15rem] md:font-extrabold">
            {title}
          </h1>
          {description && (
...
export function AuthForm({
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-5 md:block">
      {children}
    </form>
  );
}
...
export function AuthFieldRow({ children }: { children: ReactNode }) {
  return <div className="flex flex-col gap-5 md:grid md:grid-cols-2 md:gap-3.5">{children}</div>;
}
...
export function AuthCheckbox({
  ...
  return (
    <label className="flex min-h-11 items-center gap-2.5 text-sm leading-relaxed text-ink-mid md:mb-[18px] md:min-h-0 md:items-start md:gap-2 md:text-xs">
      <input
        type="checkbox"
        checked={checked}
        onChange={onChange}
